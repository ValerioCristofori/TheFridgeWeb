...
public class Queries {
	...
	public static int removeFoodInTheFridge(Statement stmt, Food food, int id) throws SQLException {
		String removeStatement = String.format("DELETE FROM Food WHERE name = '%s' and fridge = '%d';", food.getName(), id, food.getQuantity() );
	...
	public static ResultSet takeFoodQuantity(Statement stmt, String food, int id ) throws SQLException {
        String sql = String.format("SELECT quantity FROM Food WHERE name = '%s' and fridge = '%d';", food, id);
        return stmt.executeQuery(sql);
    }
